...
export class OtpService {
  ...
  async sendOtp(
    identifier: string,
    type: OtpType = OtpType.Number,
  ): Promise<{ message: string }> {
    if (!identifier) {
      throw new BadRequestException(
        `${type === OtpType.Number ? 'Phone number' : 'Email'} is required`,
      );
    }
    ...
    if (type === OtpType.Number) {
      ...
      await this._smsService.sendSms(identifier, message);
  ...
  async verifyOtp(
    ...
    if (!identifier || !otp) {
      throw new BadRequestException(
        `${type === OtpType.Number ? 'Phone number' : 'Email'} and OTP are required`,
      );
    }
  ...
  async requestVerificationOtps(
    ...
    return {
      message: 'Verification OTPs sent to both mobile number and email',
    };
  }
